...
if [[ $# -lt 3 ]]; then
  ...
  echo " --num-jobs <nj>                  # number of jobs used to split the reference data directory."
...
ref_data_dir=$1 # data directory to be used as reference for rearranging alignments
shift;
dest=$1;
shift;
first_src=$1;
...
echo "$0: splitting the ref_data_dir to get reference utt2spk for individual ali.JOB.gz files."
utils/split_data.sh $ref_data_dir $num_jobs || exit 1;
...
for i in `seq 1 $num_jobs`; do
  awk '{print $1}' $ref_data_dir/split$num_jobs/$i/utt2spk | sort > $temp_dir/utt_subset.$i
...
for i in `seq 1 $num_jobs`; do
  ...
  python -c "import sys;
percent = float($num_lines)/$num_lines_tot*100
if percent < 90 :
